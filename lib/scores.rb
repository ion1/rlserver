...
<html>
  ...
  <body>
    ...
    <table class="sort-table" id="scores">
      ...
      <tbody>#{i=0;score.data.map{|points| "<tr class=#{COLORS[i % 2]}><td>#{i+=1}</td><td align=\"right\">#{points["sc"]}</td><td>#{points["name"]}</td><td>#{points["race"]} #{points["cls"]} (lvl: #{points["xl"]})</td><td>#{points["place"]}</td><td align=\"right\">#{points["turn"]}</td><td>#{points["vmsg"]}</td></tr>"}.join("\n")}</tbody>
    </table>
    </p>
  </body>
</html>
