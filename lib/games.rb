    ...
    @play.sessions.each do |ses|
      ...
      if user && game && size && date then
        ...
        sessions << {
          ...
          :user => user,
          :game => game,
          :shortname => "#{RlConfig.config['games'][game]['name']} #{RlConfig.config["games"][game]["version"]}",
          :longname => "#{RlConfig.config['games'][game]['longname']} #{RlConfig.config["games"][game]["version"]}",
          :width => width,
          :height => height,
